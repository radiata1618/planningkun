...
final categoryImagePhotoFileProvider = ChangeNotifierProvider(
        (ref) => CategoryImagePhotoFile());
...
Future<void> insertCategory(WidgetRef ref,String categoryName) async {
  ...
  try {
    ...
    String pathStr= ref.watch(categoryImagePhotoFileProvider.notifier).categoryImagePhotoFile!.path;
    await storage.ref("categories/" + insertedDocId + pathStr.substring(pathStr.lastIndexOf('.'),))
        .putFile(ref.watch(categoryImagePhotoFileProvider.notifier).categoryImagePhotoFile!);
    //TODO 拡張子はPNGとは限らない。

    await FirebaseFirestore.instance.collection('categories').doc(insertedDocId)
        .update({"photoUpdateCnt":"1",
      "photoPath": "categories/" + insertedDocId + pathStr.substring(pathStr.lastIndexOf('.'),),
